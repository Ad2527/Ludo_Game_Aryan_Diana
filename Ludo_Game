...
class player:
    ...
    def move(self, index):  # called everytime a player needs to move
...
players.append(P1)  # initializing each player
players.append(P2)
players.append(P3)
players.append(P4)
...
def print_board():  # prints the board array when called
    table = []
    temp = []
...
def resetboard():  # resets the board and empties it
...
def addpiece(pos, count):  # called when we want to add a piece to the board
    if (pos > 98):
        return
    string = board[pos]
...
def updatepoistions():  # puts all the counters for ech player
    resetboard()
    for i in range(4):
...
def checkKill(p, pieceToMove):  # after a counter has been moved this function is called to check if it has collided
    person = players[p - 1]
    kill = False
    for x in players:
        if x.player == person.player:
            ...
            other_pos = x.getCoutners()  # other players counters
            for i in range(len(other_pos)):
                if person.counters[pieceToMove] == other_pos[i] and (
                    ...
                    person.setkills(1)  # kill is valid
...
def playTurn(p):  # called for each player object and goes through their turn systematically
    person = players[p - 1]
    ...
    if completed != 4:
        ...
        # gets the next available piece to move if others are currently stuck
        ...
        print("Player: ", p, " You have already Finished!!")
    pass


def game():  # game function, runs the game until all have finished and displays the winners at the end
    updatepoistions()
    ...
    while not allWon:
        ...
        if len(winners) == 4:
            allWon = True
    print(winners)
    print(colored("THE WINNER IS: PLAYER {} ".format(winners[0]), "red"))

game() #calls game function in main
